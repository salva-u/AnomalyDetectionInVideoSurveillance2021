#This code has been obtained from https://github.com/WaqasSultani/AnomalyDetectionCVPR2018, it has been edited to run on newer libraries
...
#    print("Loading training batch")
    ...
    Num_Normal = 798    # Total number of Normal videos in Training Dataset.
    ...
    All_Videos=sorted(listdir(AllVideos_Path))
    All_Videos.sort()
    AllFeatures = []  # To store   features of a batch
    ...
    for iv in Abnor_list_iter:
        ...
        f = open(VideoPath, "r")
        words = f.read().split()
        num_feat = int(len(words) / 2048)
        # Number of features per video to be loaded. In our case num_feat=32, as we divide the video into 32 segments. Note that
        # we have already computed   features for the whole video and divide the video features into 32 segments. Please see Save_ Features_32Segments.m as well
    ...
    for iv in Norm_list_iter:
        ...
        words = f.read().split()
        feat_row1 = np.array([])
        num_feat = int(len(words) /2048)   # Number of features to be loaded. In our case num_feat=32, as we divide the video into 32 segments.
...
AllClassPath='./Dataset/TrainingFeatures/'
# AllClassPath contains   features (.txt file)  of each video. Each text file contains 32 features, each of 2048 dimension
output_dir='./Trained_AnomalyModel/'
# Output_dir is the directory where you want to save trained weights
weights_path = output_dir + 'def_run_weights.mat'
# weights.mat are the model weights that you will get after (or during) that training
model_path = output_dir + 'def_run_model.json'
...
for it_num in range(num_iters):
    ...
    AbnormalPath = os.path.join(AllClassPath, All_class_files[0])  # Path of abnormal already computed   features
    NormalPath = os.path.join(AllClassPath, All_class_files[1])    # Path of Normal already computed   features
    inputs, targets=load_dataset_Train_batch(AbnormalPath, NormalPath)  # Load normal and abnormal video   features
