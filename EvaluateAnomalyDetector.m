...
C3D_CNN_Path='./Dataset/TestingFeatures/'; % C3D features for videos
Testing_VideoPath='./Dataset/TestingVideos/'; % Path of mp4 videos
AllAnn_Path='./Temporal_Anomaly_Annotation_for_Testing_Videos.txt'; % Path of Temporal Annotations
Model_Score_Folder='./Eval_Res/';  % Path of Pretrained Model score on Testing videos (32 numbers for 32 temporal segments)
Paper_Results='./Paper_Results/';   % Path to save results.
...
for ivideo=1:nVideos
    ...
    name = name(2:end-1)
    All_Videos_scores(ivideo).name(1:end-3)
  ...
  %% Load Video
    ...
    Predic_scores=load(ScorePath, '-ASCII');
    fps=30;
    ...
    nFrames_C3D=Actual_frames;  % As the features were computed for every 16 frames


%% 32 Shots
    Detection_score_32shots=zeros(1,nFrames_C3D);
    Thirty2_shots= round(linspace(1,nFrames_C3D,33));
    Shots_Features=[];
    p_c=0;
    ...
    for ishots=1:33
        ...
        if ee<ss
            Detection_score_32shots((ss-1)+1:(ss-1)+1+15)=Predic_scores.predictions(p_c);   
        else
